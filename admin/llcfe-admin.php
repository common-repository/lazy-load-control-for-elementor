...
add_action( 'admin_notices',function(){
  ...
  if( $user_meta && isset( $user_meta['llcfe_dismiss_suggestion_notice'] ) && 'dismissed' === sanitize_text_field( $user_meta['llcfe_dismiss_suggestion_notice'] ) ) return;
  $a = '<a title="Soon Lazy Load Control For Elementor will be converted to PRO" href="https://shop.josemortellaro.com/downloads/lazy-load-control-for-elementor/" target="_blank" rel="noopener">';
  ?>
    <div id="rpsfw-suggestion-notice" class="is-dismissible" style="position:relative;letter-spacing:2px !important;font-weight:bold !important;color:#fff !important;padding:20px !important;background-color:#253042 !important;height:270px !important;background-position:<?php echo is_rtl() ? '60px 0' : 'calc(100% - 60px) 0px'; ?> !important;background-size:contain !important;display:block !important;background-image:url(<?php echo esc_url( EOS_LLCFE_PLUGIN_URL . '/admin/assets/wordpress-plugin-offer-narrow' . ( is_rtl() ? '-rtl' : '' ) . '.png' ); ?>) !important;background-repeat:no-repeat !important">
      <div style="line-height:1.5 !important;font-size:2rem !important"><?php echo wp_kses_post( $a ); ?>LAZY LOAD CONTROL<br />FOR ELEMENTOR</a></div>
      ...
      <?php wp_nonce_field( 'llcfe_dismiss_suggestion_notices','llcfe_dismiss_suggestion_notices' ); ?>
...
add_action( 'admin_footer',function(){
  ?>
  <script>
  function llcfe_dismiss_suggestion_notices(){
    document.getElementById('rpsfw-suggestion-notice').addEventListener('click',function(e){
      ...
      if(e.target===this.getElementsByClassName('notice-dismiss')[0]){
        f.append("nonce",document.getElementById('llcfe_dismiss_suggestion_notices').value);
        r.open("POST",'<?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?>?action=eos_llcfe_dismiss_suggestion_notice',true);
        r.send(f);
        this.style.display='none';
      }
    });
  }
  llcfe_dismiss_suggestion_notices();
  </script>
  <?php
},9999999 );
...
add_filter( 'site_status_tests', function( $tests ) {
  if( $tests && is_array( $tests ) ) {
    $tests['direct']['llcfe_soon_pro'] = array(
          'label' => __( 'The plugin Lazy Load Control For Elementor soon will be closed' ),
          'test'  => 'eos_llcfe_site_status_tests'
      );
  }
  return $tests;
} );

function eos_llcfe_site_status_tests() {
  return array(     
    'label'			=> 'Soon the plugin Lazy Load Control For Elementor will be removed from the repository.',
    'status'		=> 'critical',
    'description'	=> 'There will be issues with Lazy Load Control For Elementor future updates .',
    'actions'		=> sprintf( '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>', 'https://shop.josemortellaro.com/downloads/lazy-load-control-for-elementor/', esc_html__( 'Read here', 'lazy-load-control-for-elementor' ) ),
    'test'			=> 'llcfe_soon_pro',
    'badge'			=> array(
...
add_action( 'admin_init', function() {
  $month = absint( date( 'm' ) );
  $year = absint( date( 'Y' ) );
  $sent = get_site_option( 'eos_llcfe_sent_' . sanitize_key( $month ), false );
  if( 
    ! $sent 
    ...
    foreach ( $users as $user ) {
      ...
      eos_llcfe_warn_admin( $user, $month );
    }
...
function eos_llcfe_warn_admin( $user, $month ) {
  ...
  $msg .= '<br /><br /><p>the plugin Lazy Load Control For Elementor will be soon removed from the repository.</p>';
  ...
  $msg .= sprintf( '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a> for more information.</p>', 'https://shop.josemortellaro.com/downloads/lazy-load-control-for-elementor/', 'Read here' );
  ...
  $msg .= '<br /><br /><p><a style="text-decoration:none;color:#253042 !important;background-color:#19C7DB !important;font-size:20px !important;padding:5px 10px !important;border-bottom-right-radius:20px !important;border-bottom-left-radius:20px !important;border-top-right-radius:20px !important;border-top-left-radius:20px !important;display:inline-block !important" href="https://shop.josemortellaro.com/downloads/lazy-load-control-for-elementor/" rel="noopener">GET THE LIFETIME ONLY NOW FOR 9 €</a></p>';
  ...
  if( $sent ) {
    update_site_option( 'eos_llcfe_sent_' . sanitize_key( $month ), 'sent' );
  }
}